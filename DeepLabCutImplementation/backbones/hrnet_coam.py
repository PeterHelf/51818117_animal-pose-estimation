...
from DeepLabCutImplementation.backbones.hrnet import HRNet
from DeepLabCutImplementation.modules.coam_modules import CoAMBlock, SelfAttentionModule_CoAM
from DeepLabCutImplementation.modules.keypoint_encoders import BaseKeypointEncoder
...
class HRNetCoAM(HRNet):
        ...
        if self.selfatt_coam_modules:
            for selfatt_coam_pos in self.selfatt_coam_modules:
                if selfatt_coam_pos == 4:
                    ...
                    channels = all_output_channels[selfatt_coam_pos - 1]
    ...
    def forward(self, x: torch.Tensor, cond_kpts: np.ndarray):
        ...
        cond_hm = cond_hm.float().to(x.device)
